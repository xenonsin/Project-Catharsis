...
{
    [Implements(typeof(IStandaloneInputModule), InjectionBindingScope.CROSS_CONTEXT)]
    public class StandaloneInputModule : PointerInputModule, IStandaloneInputModule
    {
        [Inject]
        ...
        {
            string inputConfigName = InputManager.GetCurrentInputConfiguration().name;
        ...
        [MenuItem("Catharsis/InputManager/Use Custom Input Module", false, 200)]
        private static void FixEventSystem()
        {
            ...
            if (im.Length > 0)
            {
                ...
                {
                    im[i].gameObject.AddComponent<Catharsis.InputEditor.UI.StandaloneInputModule>();
