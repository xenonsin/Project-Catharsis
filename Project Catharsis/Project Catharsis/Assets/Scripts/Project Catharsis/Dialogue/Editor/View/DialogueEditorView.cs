...
namespace Catharsis.DialogueEditor
{
    public class DialogueEditorView : EditorView
    {
        ...
        public enum HierarchyOptions
        {
            DialogueSelector,VariableEditor,
        }
        ...
        //For Variable Editor
        private VariableEditorScopes _scope;
        private VariableEditorTypes _type;
        private HierarchyOptions _options;
        ...
        void OnGUI()
        {
            if (loaded)
            {
                ...
                switch (_options)
                {
                        case HierarchyOptions.DialogueSelector:
                            DisplayHierarchyPanel();
                        break;
                        case HierarchyOptions.VariableEditor:
                            DisplayVariableEditor();
                        break;
                }
        ...
        private void DisplayEditorChoice()
        {
            Rect screenRect = new Rect(0.0f, _toolbarHeight + 5.0f, _hierarchyPanelWidth, 30);
        ...
        #region ViriableEditorPanel


        void DisplayVariableEditor()
        {
            DialogueEditorVariablesContainer variables = getVariables();

            int globalFloatsCount = _dialogueData.data.globals.floats.variables.Count;
            int globalBooleansCount = _dialogueData.data.globals.booleans.variables.Count;
            int globalStringsCount = _dialogueData.data.globals.strings.variables.Count;

            int localFloatsCount = _dialogueData.data.dialogues[currentDialogue].floats.variables.Count;
            int localBooleansCount = _dialogueData.data.dialogues[currentDialogue].booleans.variables.Count;
            int localStringsCount = _dialogueData.data.dialogues[currentDialogue].strings.variables.Count;
            ...
            int currentFloatsCount = (_scope == VariableEditorScopes.Local) ? localFloatsCount : globalFloatsCount;
            ...
            GUILayout.BeginHorizontal();           
            if (GUILayout.Toggle(_scope == VariableEditorScopes.Global, "Globals (" + globalVarsCount + ")", EditorStyles.miniButtonLeft ))
                setTarget(VariableEditorScopes.Global, _type);
            if (GUILayout.Toggle(_scope == VariableEditorScopes.Local, "Locals (" + localVarsCount + ")", EditorStyles.miniButtonRight))
                setTarget(VariableEditorScopes.Local, _type);
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            if (GUILayout.Toggle(_type == VariableEditorTypes.Boolean, "Booleans (" + currentBooleansCount + ")", EditorStyles.miniButtonLeft))
                setTarget(_scope, VariableEditorTypes.Boolean);
            if (GUILayout.Toggle(_type == VariableEditorTypes.Float, "Floats (" + currentFloatsCount + ")", EditorStyles.miniButtonMid))
                setTarget(_scope, VariableEditorTypes.Float);
            if (GUILayout.Toggle(_type == VariableEditorTypes.String, "Strings (" + currentStringsCount + ")", EditorStyles.miniButtonRight))
                setTarget(_scope, VariableEditorTypes.String);
            GUILayout.EndHorizontal();
            ...
            {
                GUI.color = new Color(1, 1, 1, 0.5f);
                GUILayout.Button("Remove", EditorStyles.miniButtonRight);
                GUI.color = GUI.contentColor;
            }
            ...
            {
                mouseClickPosition = new Vector2(Event.current.mousePosition.x - scrollRect.x - 3, Event.current.mousePosition.y - scrollRect.y  + _variableScrollPos.y);
               // Debug.Log(mouseClickPosition);

            }
            GUILayout.BeginArea(scrollRect);
            _variableScrollPos = GUILayout.BeginScrollView(_variableScrollPos);
            for (int i = 0; i < variables.variables.Count; i++)
            {
                Rect row = new Rect(0, 0 + ((rowHeight + -1) * i), scrollRect.width, 20);
                if (mouseClickPosition != Vector2.zero && row.Contains(mouseClickPosition))
                    variables.selection = i;

                if (variables.selection == i)
                {
                    if (_highlightTexture == null)
                    {
                        CreateHighlightTexture();
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Label(variables.variables[i].id.ToString(), GUILayout.Width(30));
                string labelNameText = (variables.variables[i].variableName != string.Empty) ? variables.variables[i].variableName : string.Empty;
                //GUILayout.Label(labelNameText);
                //GUI.color = new Color(1, 1, 1, 0.5f);
                GUILayout.Label((variables.variables[i].variable != string.Empty) ? labelNameText + ": " + variables.variables[i].variable : string.Empty);
                //GUI.color = GUI.contentColor;
                GUILayout.EndHorizontal();
            }
        ...
        {
            if (_scope == scope && _type == type) return;
        ...
        private void DrawEditor()
        {
		    ...
		    if(variables.variables.Count < 1){
			    DrawNoVariablesWarning( _scope, _type);
		    }else{
			    switch(_type){
				    case VariableEditorTypes.Boolean:
					    DrawBooleanEditor(variables);
				    break;
				
				    case VariableEditorTypes.Float:
					    DrawFloatEditor(variables);
				    break;
				
				    case VariableEditorTypes.String:
					    DrawStringEditor(variables);
				    break;
			    }
		    }
        ...
        private DialogueEditorVariablesContainer getVariables()
        {
            ...
            if (_scope == VariableEditorScopes.Global)
            {
                if (_type == VariableEditorTypes.Float)
                {
                    variables = _dialogueData.data.globals.floats;
                }
                else if (_type == VariableEditorTypes.String)
                {
        ...
        public void DrawNoVariablesWarning(VariableEditorScopes scope, VariableEditorTypes type)
        {
            ...
            GUILayout.Label("No " + scope.ToString() + " " + type.ToString() + " found.\n\n", centered);
        }
        public void DrawBooleanEditor(DialogueEditorVariablesContainer variables)
        {
            string targetName = _scope.ToString() + " " + _type.ToString();


            GUILayout.Label(targetName + " Name");
            variables.variables[variables.selection].variableName = GUILayout.TextField(variables.variables[variables.selection].variableName, LargeTextFieldStyle());

            GUILayout.Label(targetName + " Value");

            GUILayout.BeginHorizontal();
            if (GUILayout.Toggle((variables.variables[variables.selection].variable == "true"), "True", EditorStyles.miniButtonLeft)) variables.variables[variables.selection].variable = "true";
            if (GUILayout.Toggle((variables.variables[variables.selection].variable != "true"), "False", EditorStyles.miniButtonRight)) variables.variables[variables.selection].variable = "false";
            GUILayout.EndHorizontal();
        }

        public void DrawFloatEditor(DialogueEditorVariablesContainer variables)
        {
            ...
            variables.variables[variables.selection].variable = EditorGUILayout.FloatField(floatVar, LargeTextFieldStyle()).ToString();
        }

        public void DrawStringEditor(DialogueEditorVariablesContainer variables)
        {
            string targetName = _scope.ToString() + " " + _type.ToString();
            ...
            GUILayout.Label(targetName + " Value");
            variables.variables[variables.selection].variable = GUILayout.TextField(variables.variables[variables.selection].variable, LargeTextFieldStyle());
        }

        private GUIStyle LargeTextFieldStyle()
        {
        ...
        #endregion
        #region DialogueSelector
        void DisplayHierarchyPanel()
        {
            Rect screenRect = new Rect(0.0f, _toolbarHeight + 20.0f, _hierarchyPanelWidth, position.height - _toolbarHeight + 10.0f);
            //Rect buttonView = new Rect(screenRect.x, screenRect.y + 20,screenRect.width, position.height - screenRect.y);
            Rect scrollView = new Rect(screenRect.x, screenRect.y +10.0f, screenRect.width, position.height - screenRect.y );
            //ADD
        ...
        void DisplayHierarchyDialogueItem(Rect rect, int index, string name)
        {
            ...
            {
                mouseClickPosition = new Vector2(Event.current.mousePosition.x - rect.x - 3, Event.current.mousePosition.y - rect.y + 17 + _hierarchyItemHeight);
            }
